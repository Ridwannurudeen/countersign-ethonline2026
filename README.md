...
advance and the service cannot alter. A fixed number of single-approval nonces is therefore a
...
with no change to the seller. On this buyer path, the mandate authorizes the payee and caps
the transfer amount; it does not bind the resource, the quoted price or delivery. Two runs are recorded on
...
The table above is one representative approved/refused pair. The guard created a new HCS
topic per process start until the topic was pinned, fragmenting verdict history across
topics. The guard created a new topic on every restart until the topic was pinned; its
creation history contains 13 topics, of which seven hold verdicts. The pinned topic is
`0.0.10511981` and all current verdicts land there.
Each link below lists the complete message history for that topic:

- [Topic `0.0.10502379`](https://testnet.mirrornode.hedera.com/api/v1/topics/0.0.10502379/messages)
- [Topic `0.0.10502545`](https://testnet.mirrornode.hedera.com/api/v1/topics/0.0.10502545/messages)
- [Topic `0.0.10502767`](https://testnet.mirrornode.hedera.com/api/v1/topics/0.0.10502767/messages)
- [Topic `0.0.10506561`](https://testnet.mirrornode.hedera.com/api/v1/topics/0.0.10506561/messages)
- [Topic `0.0.10507040` (buyer path)](https://testnet.mirrornode.hedera.com/api/v1/topics/0.0.10507040/messages)
- [Topic `0.0.10511981` (pinned; all tenants)](https://testnet.mirrornode.hedera.com/api/v1/topics/0.0.10511981/messages)

`web/evidence.json` is the checked-in schedule-run manifest, regenerated by
`npm run build-evidence`; it is not a complete index of these topic histories.
...
Metering is deployed and live on the transfer path; the schedule path stays flat.
...
The deployed instance at `countersign.gudman.xyz` serves **two operator-enrolled tenants**: the treasury
...
**HTS approval differs by route.** On the schedule path every token proposal is refused: invalid
...
Mandates permit a single approval per tenant. Refusals do not reserve the nonce, so a
refused mandate remains usable until it expires, subject to the nonce high-water mark.
Before approval, the guard atomically reserves
