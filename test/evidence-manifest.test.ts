...
test("the produced JSON satisfies the evidence page contract", async () => {
  ...
  // Matching a parse and re-serialise proves it was never hand-edited, so its counts
  // cannot overstate what actually happened on chain. Line endings are normalised
  // because a Windows checkout rewrites the committed LF to CRLF.
  assert.equal(
    `${JSON.stringify(checkedIn, null, 2)}\n`,
    raw.replace(/\r\n/g, "\n"),
  );
